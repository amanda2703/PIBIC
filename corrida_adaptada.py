...
# Com adaptações propostas por Saa, Siqueira
# Modeling the ATP Production in Mitochondria (DOI: 10.1007/s11538-013-9862-1)
...
kellen = {'VmaxANT':5,'Atot':15e3,'alfa_c':0.111,'alfa_m':0.139,'F_RT':0.037,'f':0.5}
mk = {'Kcat':6.0,'Kact':0.38,'Jmaxuni':400,'dpuni':91,'nact':2.8,'Lunimax':50,'KNa':9.4,'KCa':0.003,'Nai':30,'dpNaCa':91,'b':0.5,'JmaxNaCa':5.5,'n':3}
parametros.update(aux1) 
parametros.update(aux2)
parametros.update(aux3)
parametros.update(kellen)
parametros.update(mk)

# Fluxos
def find_Jant(ADPm,delta_psi,parametros):
    ADPm = ADPm*1000
    VmaxANT = parametros['VmaxANT']
    Atot = parametros['Atot']
    alfa_m = parametros['alfa_m']
    alfa_c = parametros['alfa_c']
    F_RT = parametros['F_RT']
    f = parametros['f']
    ATPm = Atot-ADPm
    Jant = VmaxANT*(ATPm/(Atot-((1-alfa_m)*ATPm)))*((alfa_m-(alfa_c*np.exp(-1*F_RT*delta_psi)))/(1+(alfa_c*(ATPm/ADPm)*np.exp(-1*f*F_RT*delta_psi))))
    return Jant
...
def find_Juni(Cai,dp,parametros):
    F = 96485
    R = 8314
    T = 310
    Kcat = parametros['Kcat']
    Kact = parametros['Kact']
    Jmaxuni = parametros['Jmaxuni']
    dpuni = parametros['dpuni']
    nact = parametros['nact']
    Lunimax = parametros['Lunimax']
    FRT = F/(R*T)
